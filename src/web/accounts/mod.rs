...
use app::actix_web::web::{get, post, resource, scope, ServiceConfig};
...
pub fn configure(config: &mut ServiceConfig) {
    config.service(scope("/accounts/")
        .service(resource("/register/")
            .route(get().to(views::register::form))
            .route(post().to(views::register::create_account))
        )

        .service(resource("/reset/{uidb64}-{ts}-{token}/")
        ...
        .service(resource("/verify/{uidb64}-{ts}-{token}/")
            .route(get().to(views::verify::with_token))
        )
        
        .service(resource("/verify/")
            .route(get().to(views::verify::verify))
        )

        .service(resource("/logout/")
            .route(post().to(views::logout))
        )
    );
}